...
window.CHATGPT_LOCALES.en = {
  ...
  voiceUnsupported: 'Voice input is not supported by this browser.', imageOnly: 'This version supports image attachments for Gemini vision.', imageTooLarge: '{name} is larger than 10 MB.', analyzeImages: 'Please analyze the attached image(s).', noResponse: 'No response received.', backendNotConfigured: 'Chat backend is not configured for GitHub Pages. Deploy the Node.js server and set its URL in public/config.js.', backendError: 'The chat backend returned an error ({status}). Check the backend URL and deployment.'
};
